	...
	def MakeEPG(self, prefix, channel_list=None):
		...
		for channel in data['channels']:
			...
			for epg in channel['programs']:
				startTime = datetime.datetime.utcfromtimestamp(float(epg['startTime'])/1000.+3600*9).strftime('%Y%m%d%H%M%S')
				endTime = datetime.datetime.utcfromtimestamp(float(epg['endTime'])/1000.+3600*9).strftime('%Y%m%d%H%M%S')
